/* Penalty Family PRO v2 - skill-based peer-to-peer penalty game */
...
$('#play-again').onclick=()=>{if(!state)return;if(isHost||demo){const joined=clone(state.players);const mode=state.mode;state=freshState(mode);state.players=joined;state.status='playing';state.seq++;setState(state)}else conn?.send({type:'REMATCH'})};
...
function setupHost(){
  ...
  peer.on('error',e=>{$('#room-status').textContent='خطا در ساخت اتاق. دوباره تلاش کن.';console.error(e)});
  updateReadyUI();
}
function setupGuest(){
  $('#room-status').textContent='در حال اتصال…';
  peer=new Peer();
...
function handleMessage(msg){
  ...
  if(msg.type==='REMATCH'&&isHost){const joined=clone(state.players);const mode=state.mode;state=freshState(mode);state.players=joined;state.status='playing';setState(state);show('game')}
}
...
function applyAction(action,role){
  if(!state||state.status!=='playing'||resolutionLocked)return;
  const shooter=state.turn,keeper=other(shooter);
...
function resolveIfReady(){
  const c=state.current;if(!c.shot||!c.keeper||resolutionLocked)return;
  ...
  const event={type:'resolution',seq:state.seq,shooter,keeper,shot:c.shot,keeperAction:c.keeper,result};
  broadcast();sendEvent(event);playResolution(event);
  setTimeout(()=>{resolutionLocked=false;advanceTurn()},2700);
}
...
function resolvePro(shot,keeper){
  const overhit=Math.max(0,(shot.power-.9)/.1), underhit=Math.max(0,(.28-shot.power)/.28);
  const instability=(1-shot.consistency)*.17 + overhit*.13 + underhit*.08;
  const rx=(Math.random()-.5)*2*instability, ry=(Math.random()-.5)*1.4*instability;
  const finalX=shot.targetX + shot.curve*.16 + rx;
  const finalY=shot.targetY + ry - underhit*.08;
  ...
  if(nearPost&&Math.random()<.68)return {goal:false,kind:'post',finalX,finalY,save:false};
  if(!insideX)return {goal:false,kind:'wide',finalX,finalY,save:false};
  if(!insideY)return {goal:false,kind:finalY>1?'high':'weak',finalX,finalY,save:false};
  const kp=ZONE_POS[keeper.zone]||ZONE_POS.BC;
  const dx=(finalX-kp.x),dy=(finalY-kp.y)*1.15,dist=Math.hypot(dx,dy);
  const reaction=keeper.reactionMs;
  const reactionBonus=reaction<260?.12:reaction<480?.075:reaction<750?.02:reaction<1050?-.055:-.13;
  const shotDifficulty=shot.power*.10+Math.abs(shot.curve)*.09+(shot.type==='chip'?.04:0);
  const reach=clamp(.47+reactionBonus-shotDifficulty,.22,.60);
  let save=dist<reach;
  if(!save&&dist<reach+.11)save=Math.random()<clamp(.48-(dist-reach)*2.8,0,.48);
  return {goal:!save,kind:save?'saved':'goal',finalX,finalY,save,dist,reach};
}
...
function playResolution(event){
  if(!event||event.type!=='resolution')return;
  animateBallProfile(event.shot,event.result,true);
  const k=$('#keeper');k.className='character keeper dive-'+event.keeperAction.zone;
  const phrase=resultPhrase(event);callout(phrase);speak(phrase);
  if(event.result.goal){sfx('#goal-sfx');buzz([35,40,60]);makeConfetti($('#confetti'),50);const s=$('#shooter');if(event.shooter==='sepanta'){s.classList.add('celebrate-sepanta');setTimeout(()=>speak('سییی!'),720)}else{s.classList.add('celebrate-baba')}}
  else{sfx('#save-sfx');buzz(45)}
}
...
function render(){
  ...
  const shooter=state.turn,keeper=other(shooter);$('#shooter-img').src=IMG[shooter];$('#keeper-img').src=IMG[keeper];
  const seqChanged=lastRenderSeq!==state.seq;
  if(seqChanged){lastRenderSeq=state.seq;$('#keeper').className='character keeper';$('#shooter').className='character shooter';$('#confetti').innerHTML='';resetBall();resetHud();keeperReactStart=null;keeperReactSeq=-1;lastIncomingShotKey=''}
  const myIsShooter=demo?true:myRole===shooter;const myIsKeeper=demo?false:myRole===keeper;
  $('#pro-hud').classList.toggle('hidden',state.mode!=='pro'||!myIsShooter);
  $('#aim-guide').classList.toggle('hidden',!myIsShooter||!!state.current.shot);
  $('#keeper-controls').classList.toggle('hidden',!myIsKeeper||!!state.current.keeper);
  $('#waiting').classList.toggle('hidden',demo||(myIsShooter&&!state.current.shot)||(myIsKeeper&&!state.current.keeper));
  if(state.mode==='pro'&&myIsShooter&&!state.current.shot)$('#aim-guide').textContent='سوایپ رو به بالا = ارتفاع • قوس انگشت = کات';else $('#aim-guide').textContent='توپ را رو به دروازه سوایپ کن';
  if(myIsKeeper&&!state.current.keeper){
    if(state.current.shot){const key=state.seq+':'+JSON.stringify(state.current.shot);if(lastIncomingShotKey!==key){lastIncomingShotKey=key;keeperReactStart=performance.now();keeperReactSeq=state.seq;animateBallProfile(state.current.shot,null,false)}$('#keeper-hint').textContent='شوت شد! سریع شیرجه بزن';}
    else $('#keeper-hint').textContent='آماده باش؛ جهت را پیش‌بینی کن';
  }
...
function resetBall(){const b=$('#ball');b.getAnimations?.().forEach(a=>a.cancel());b.style.left='50%';b.style.bottom='8%';b.style.top='auto';b.style.transform='translateX(-50%) scale(1) rotate(0deg)'}
...
function startDrag(e){if(!state||state.status!=='playing'||resolutionLocked)return;const shooter=state.turn;if(!demo&&myRole!==shooter)return;if(state.current.shot)return;e.preventDefault();dragStart=point(e);dragStartedAt=dragStart.t;dragPath=[dragStart];window.addEventListener('touchmove',moveDrag,{passive:false});window.addEventListener('mousemove',moveDrag);window.addEventListener('touchend',endDrag,{once:true});window.addEventListener('mouseup',endDrag,{once:true})}
...
function endDrag(e){window.removeEventListener('touchmove',moveDrag);window.removeEventListener('mousemove',moveDrag);if(!dragStart)return;const p=point(e);dragPath.push(p);const start=dragStart;dragStart=null;const dx=p.x-start.x,dy=p.y-start.y,dist=Math.hypot(dx,dy),duration=Math.max(80,p.t-dragStartedAt);if(dist<34){callout('محکم‌تر سوایپ کن',900);return}
  const profile=state.mode==='pro'?profileFromGesture(start,p,dragPath,duration):funProfile(dx,dy,dist,duration);
  updateHud(profile);animateBallProfile(profile,null,false);submitAction({kind:'shot',...profile});if(demo)setTimeout(()=>demoKeeper(profile),520)
}
...
function profileFromGesture(start,end,path,duration){
  const dx=end.x-start.x,dy=end.y-start.y,dist=Math.hypot(dx,dy),speed=dist/duration;
  const up=-dy;
  const curveInfo=measureCurve(start,end,path);
  let targetX=clamp(dx/150,-1.28,1.28);
  let targetY=clamp((up-42)/180,-.12,1.22);
  const power=clamp((dist/285)*.56+(speed/1.35)*.44,0,1);
  const curve=clamp(curveInfo.curve,-1,1);
  const consistency=clamp(1-curveInfo.scatter,0,1);
  targetX=clamp(targetX+curve*.12,-1.35,1.35);
  let type='drive';if(targetY>.78&&power<.63)type='chip';else if(Math.abs(curve)>.28)type='curve';else if(power>.82)type='power';else if(power<.57)type='placed';
  const zone=nearestZone(targetX,targetY);
  return {targetX,targetY,power,curve,consistency,type,zone,duration};
}
function measureCurve(start,end,path){const vx=end.x-start.x,vy=end.y-start.y,len=Math.hypot(vx,vy)||1;let sum=0,maxSigned=0,absSum=0,n=0;for(const p of path){const px=p.x-start.x,py=p.y-start.y;const signed=(vx*py-vy*px)/len;sum+=signed;absSum+=Math.abs(signed);if(Math.abs(signed)>Math.abs(maxSigned))maxSigned=signed;n++}const avg=n?sum/n:0;const scatter=clamp((absSum/Math.max(1,n))/95,0,1);return {curve:clamp((maxSigned*.65+avg*.35)/62,-1,1),scatter}}
...
function animateBallProfile(profile,result,finalPass){
  const b=$('#ball'),goal=$('#goal'),stad=$('#stadium');if(!b||!goal||!stad)return;
  const gr=goal.getBoundingClientRect(),sr=stad.getBoundingClientRect();
  const fx=result?.finalX??profile.targetX,fy=result?.finalY??profile.targetY;
  const endX=(gr.left-sr.left)+gr.width*(.5+fx*.46),endY=(gr.top-sr.top)+gr.height*(.9-fy*.78);
  const startX=sr.width*.5,startY=sr.height*.90;
  const curvePx=profile.curve*90;
  const duration=Math.round(830-profile.power*330);
  b.getAnimations?.().forEach(a=>a.cancel());b.style.left=startX+'px';b.style.top=startY+'px';b.style.bottom='auto';b.style.transform='translate(-50%,-50%) scale(1)';
  if(b.animate){b.animate([{left:startX+'px',top:startY+'px',transform:'translate(-50%,-50%) scale(1) rotate(0deg)'},{left:(startX+(endX-startX)*.56+curvePx)+'px',top:(startY+(endY-startY)*.48)+'px',transform:'translate(-50%,-50%) scale(.72) rotate(380deg)',offset:.55},{left:endX+'px',top:endY+'px',transform:'translate(-50%,-50%) scale(.48) rotate(760deg)'}],{duration,easing:'cubic-bezier(.15,.72,.22,1)',fill:'forwards'});}else{b.style.left=endX+'px';b.style.top=endY+'px';b.style.transform='translate(-50%,-50%) scale(.48)'}
}
...
function resetHome(){try{conn?.close();peer?.destroy()}catch{}conn=null;peer=null;state=null;roomCode=null;isHost=false;demo=false;resolutionLocked=false;keeperReactStart=null;lastRenderSeq=null;show('home')}
